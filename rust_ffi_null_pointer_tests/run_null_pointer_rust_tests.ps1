...
$ObjectFile = "null_pointer_rust.o"
$Tests = @(
...
try {
    ...
    Write-Host "> rustc +stable-x86_64-pc-windows-gnu --emit=obj --crate-type=lib -C opt-level=2 -C panic=abort --cfg null_pointer_no_main null_pointer.rs -o $ObjectFile" -ForegroundColor DarkGray
    rustc +stable-x86_64-pc-windows-gnu --emit=obj --crate-type=lib -C opt-level=2 -C panic=abort --cfg null_pointer_no_main null_pointer.rs -o $ObjectFile
